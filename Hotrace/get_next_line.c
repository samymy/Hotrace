...
static char	*ft_realloc(size_t new_size, size_t old_size, char *ptr)
{
	char	*tmp;
	...
	if (new_size <= old_size)
		return (ptr);
	if (!ptr)
		return ((char *)ft_memalloc(new_size + 1));
...
static int	ft_check(char *buf, char **line)
{
	...
	if (buf[0])
	{
		...
		{
			*line = ft_realloc(len + ptr - buf + 1, len, *line);
			*line = ft_strncat(*line, buf, ptr - buf);
			(*line)[len + ptr - buf] = '\0';
			buf = ft_strncpy(buf, ptr + 1, BUFF_SIZE + 1);
			return (1);
		}
		*line = ft_realloc(len + ft_strlen(buf) + 1, len, *line);
